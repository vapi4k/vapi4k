...
@Serializable
data class AnthropicBedrockModelDto(
  var model: String = "",
  @Transient
  override var modelType: AnthropicBedrockModelType = AnthropicBedrockModelType.UNSPECIFIED,
  @Transient
  ...
  fun verifyValues() {
    if (modelType.isSpecified() && customModel.isNotEmpty())
      error("anthropicBedrockModel{} cannot have both modelType and customModel values")

    if (modelType.isNotSpecified() && customModel.isEmpty())
      error("anthropicBedrockModel{} must have either a modelType or customModel value")
  }
}
