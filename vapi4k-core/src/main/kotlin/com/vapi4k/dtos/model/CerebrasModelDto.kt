...
import com.vapi4k.dsl.model.CerebrasModelProperties
...
@Serializable
data class CerebrasModelDto(
  var model: String = "",
  @Transient
  override var modelType: CerebrasModelType = CerebrasModelType.UNSPECIFIED,
  @Transient
  ...
  fun verifyValues() {
    if (modelType.isSpecified() && customModel.isNotEmpty())
      error("cerebrasModel{} cannot have both modelType and customModel values")

    if (modelType.isNotSpecified() && customModel.isEmpty())
      error("cerebrasModel{} must have either a modelType or customModel value")
  }
}
