...
@Serializable
data class DeepSeekModelDto(
  var model: String = "",
  @Transient
  override var modelType: DeepSeekModelType = DeepSeekModelType.UNSPECIFIED,
  @Transient
  ...
  fun verifyValues() {
    if (modelType.isSpecified() && customModel.isNotEmpty())
      error("deepSeekModel{} cannot have both modelType and customModel values")

    if (modelType.isNotSpecified() && customModel.isEmpty())
      error("deepSeekModel{} must have either a modelType or customModel value")
  }
}
