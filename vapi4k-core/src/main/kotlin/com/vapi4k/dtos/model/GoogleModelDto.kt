...
import com.vapi4k.dsl.model.GoogleModelProperties
...
@Serializable
data class GoogleModelDto(
  var model: String = "",
  @Transient
  override var modelType: GoogleModelType = GoogleModelType.UNSPECIFIED,
  @Transient
  ...
  fun verifyValues() {
    if (modelType.isSpecified() && customModel.isNotEmpty())
      error("googleModel{} cannot have both modelType and customModel values")

    if (modelType.isNotSpecified() && customModel.isEmpty())
      error("googleModel{} must have either a modelType or customModel value")
  }
}
